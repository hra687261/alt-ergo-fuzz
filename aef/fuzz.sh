...
EXEC="$DIR"/../_build/default/aef/main_afl.exe
...
if $TOFILES; then
  if [[ ( "$NBCORES" > 1 ) ]]; then
    (afl-fuzz -t $TIMEOUT -m $MEMLIMIT -i $INPUTDIR -o $SYNC_DIR -M fuzzer0 $EXEC @@) > "$COPF"0.txt 2>&1 & echo $! > $IDF
    for ((i = 1; i < $NBCORES; i++ )); do
      afl-fuzz -t $TIMEOUT -m $MEMLIMIT -i $INPUTDIR -o $SYNC_DIR -S fuzzer$i $EXEC @@ > "$COPF"$i.txt 2>&1 & echo $! >> $IDF
    done
  else
    afl-fuzz -t $TIMEOUT -m $MEMLIMIT -i $INPUTDIR -o ./aef/output/ $EXEC @@ > "$COPF"0.txt 2>&1 & echo $! > $IDF
  fi
else
  if [[ ( "$NBCORES" > 1 ) ]]; then
