  ...
  let g2 =
    fun {fn; params = (p1, p2, p3, p4, p5); rtyp} ->
      ...
      Cr.map [
        ...
        fun a1 a2 a3 a4 a5 ->
          let args, u_args, u_bvars, u_dt, c_funcs = 
            List.fold_left (
              fun (l, vs, bv, ud, fcs) b -> 
                if is_dummy b.g_res
                then l, vs, bv, ud, fcs
                else 
                  b.g_res :: l, 
            ...
            ) ([], VS.empty, VS.empty, SS.empty, SS.empty) 
              [a1; a2; a3; a4; a5]
          in
          { g_res = FunCall {
                fname = fn; fk = USF; 
      ...
      fun a1 a2 a3 a4 a5 ->
        let args, u_args, u_bvars, u_dt, c_funcs = 
          List.fold_left (
            ...
            ([], VS.empty, VS.empty, SS.empty, SS.add fname SS.empty) 
            [a1; a2; a3; a4; a5]
        in
        { g_res = FunCall {
              fname; fk = UDF; 
    ...
    | [(n1, t1); (n2, t2); (n3, t3); (n4, t4); (n5, t5)] -> 
      Cr.map [ 
        ...
        fun 
          ...
          in
          let rparams, u_args, u_bvars, u_dt, c_funcs = 
            List.fold_left (
              fun (pl, ua, ub, ud, cf) 
                (n, {g_res; u_args; u_bvars; u_dt; c_funcs}) -> 
                (n, g_res) :: pl, 
                VS.union u_args ua, 
                VS.union u_bvars ub, 
                SS.union u_dt ud, 
                SS.union c_funcs cf
            ) ([n1, g_res], u_args, u_bvars, u_dt, c_funcs) tmp
          in
...
let fdef_gen ?(fdefs = []) ?(adts : adt list = []) 
  ...
  let fg = 
    fun ((t1, t2, t3, t4, t5), rtyp) ->
      ...
      let ge =
        Cr.map [gen] ( 
          fun {g_res = body; u_args; u_bvars; u_dt; c_funcs} ->
            let atyp = 
              List.map (
                fun v -> 
                  if VS.mem v u_args
    ...
    ( fun {g_res; u_args; u_bvars; u_dt; c_funcs} ->
        let g_res =
          Goal {
            ...
            body = quantify g_res}
        in 
        {g_res; u_args; u_bvars; u_dt; c_funcs}
    )
